    ...
    switch (platform) {
      case 'facebook':
        if (isMobile) {
          ...
          shareUrl = `https://www.facebook.com/sharer/sharer.php?href=${encodeURIComponent(url)}`;
        }
        break;
      case 'reddit':
        if (isMobile) {
          ...
          shareUrl = `https://www.reddit.com/submit?url=${encodeURIComponent(url)}&title={frontmatter.title}`
        }
        break;
      case 'twitter':
